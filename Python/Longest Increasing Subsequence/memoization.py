    ...
    for i in range(1, arr_len):
        for j in range(i):
            if arr[j]<=arr[i]:   ## Current element is greater than iterator element, then, well, it CAN be included in LIS
                already_existing_solution = cache[i]
                new_answer = cache[j]+1
                if new_answer>already_existing_solution: ## But only include it if it produce a higher  number (LIS) than already existing one i.e. cache[j]>cache[i]
                    cache[i] = new_answer
